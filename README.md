...
- Unified account management for Base URL, API Key, platform, routes, downstream keys, and proxy logs.
- Internal site/upstream create/edit APIs remain for compatibility, while the main admin flow creates or reuses upstream config from the account page.
...
- Account create and edit right-side drawer UI for Base URL, API Key, platform, credentials, account-level proxy, unit cost, status, pinned state, and sort order.
- Account batch enable/disable/delete and selected balance refresh from the admin UI.
- Account API Key is stored as an internal account credential; the admin UI no longer exposes a separate credential workflow.
- Downstream key policy controls for model scope, route/site/account/internal-credential authorization, exclusions, site weight multipliers, and batch operations.
- Downstream key request and cost usage accounting, with account `unitCost` used for usage-based cost estimates and admin usage reset.
- Account-level proxy configuration stored per account and preferred before site or system proxy.
- Internal account credential compatibility for OpenAI-compatible New API / One API style token lists.
...
- Native account availability monitor with scheduler, heartbeats, uptime windows, manual checks, and notification events.
...
- OAuth account list, enable/disable, refresh/quota, import, and delete APIs, surfaced in the admin UI.
- Stats overview API and dashboard cards for today's requests, success rate, tokens, cost, active upstreams, and abnormal accounts.
...
- Native monitor page for account status, heartbeat bars, uptime, manual checks, and monitor settings.
...
3. Open the account page and add an account with `baseUrl` and `apiKey`.
4. Refresh account models.
5. Rebuild routes.
6. Create a downstream key with `modelScope = all`.
7. Call `/v1/models` or `/v1/chat/completions` using that key.
